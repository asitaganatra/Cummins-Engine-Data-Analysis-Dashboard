...
# --- Helper to display IDs in a grid to avoid scrolling ---
def display_id_grid(id_list, title):
    st.markdown(f"**{title} ({len(id_list)} Unique IDs)**")
    if len(id_list) > 0:
        # Dynamically create rows of 10 to fit the screen width
        rows = [id_list[i:i + 10] for i in range(0, len(id_list), 10)]
        grid_df = pd.DataFrame(rows)
        st.table(grid_df)
    else:
        st.info("No unique IDs isolated in this comparison.")
...
if uploaded_files:
    ...
    if df_result is not None and len(df_result) > 0:
        ...
        if file_type == "Raw CAN Log" and len(uploaded_files) > 1:
            ...
            can_tab1, can_tab2, can_tab3 = st.tabs(["📊 Traffic Overview", "🔍 Manual ID Comparison", "🧪 Advanced Re-Analysis"])
            ...
            with can_tab2:
                st.header("🔍 Manual ID Comparison & Export")
                st.info("Compare 'Master' (All Sensors) vs 'Filtered' (Without Sensor) to isolate the target ID.")
                file_names = [f.name for f in uploaded_files]
                col_sel1, col_sel2 = st.columns(2)
                f1_name = col_sel1.selectbox("Select File 1 (Master Log):", file_names, index=0)
                f2_name = col_sel2.selectbox("Select File 2 (WITHOUT Sensor Log):", file_names, index=1 if len(file_names)>1 else 0)

                # Load and extract IDs for the two selected files
                df1, _ = load_data(uploaded_files[file_names.index(f1_name)])
                df2, _ = load_data(uploaded_files[file_names.index(f2_name)])
                
                ids1 = sorted(df1['ID'].unique())
                ids2 = sorted(df2['ID'].unique())

                display_id_grid(ids1, f"IDs in {f1_name}")
                display_id_grid(ids2, f"IDs in {f2_name}")

                st.divider()
                # LOGIC: Master - Filtered = Isolated Target IDs
                diff_ids = sorted(list(set(ids1) - set(ids2)))
                
                st.subheader("✨ Isolated Unique IDs (Temperature Sensors)")
                if diff_ids:
                    display_id_grid(diff_ids, "Unique IDs found in Master but absent in Filtered")
                    
                    st.markdown("### 📥 Record & Export Results")
                    export_name = st.text_input("Name your exported sensor key:", value="Temperature_Sensor_Key")
                    
                    csv_data = pd.DataFrame({"Sensor IDs": diff_ids}).to_csv(index=False).encode('utf-8')
                    st.download_button(
                        label="💾 Download Sensor Key as CSV",
                        data=csv_data,
                        file_name=f"{export_name}.csv",
                        mime='text/csv'
                    )
                else:
                    st.info("No unique IDs found: File 2 contains all IDs found in File 1.")

            with can_tab3:
                st.header("🧪 Advanced Re-Analysis")
                st.info("Upload your exported Sensor Key and a NEW Mixed Log to find new/unknown sensors.")
                
                col_up1, col_up2 = st.columns(2)
                ref_file = col_up1.file_uploader("Upload Saved Sensor Key (CSV):", type=["csv"], key="ref_key")
                new_file = col_up2.file_uploader("Upload New Mixed Log to Compare:", type=["csv"], key="new_mix")

                if ref_file and new_file:
                    df_ref = pd.read_csv(ref_file)
                    df_new, _ = load_data(new_file)
                    
                    # LOGIC: New Mixed File - Known Sensor Key = Discovered New IDs
                    known_ids = set(df_ref.iloc[:, 0].astype(str).unique())
                    current_ids = set(df_new['ID'].unique())
                    
                    new_discovered_ids = sorted(list(current_ids - known_ids))
                    
                    if new_discovered_ids:
                        st.success(f"Isolated {len(new_discovered_ids)} NEW Unknown/Other Sensor IDs.")
                        display_id_grid(new_discovered_ids, "Discovered New Sensor IDs")
                    else:
                        st.info("All IDs in the new log match your known Sensor Key.")
